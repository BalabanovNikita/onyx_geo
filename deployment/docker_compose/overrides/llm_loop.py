...
def construct_message_history(
    ...
    # 4. Add messages after last user message (tool calls, responses, etc.)
    result.extend(messages_after_last_user)
    
    # 5. Add reminder message
    if reminder_message:
        result.append(reminder_message)

    # 6. Add custom agent prompt (inserted before last user message)
    if custom_agent_prompt:
        result.append(custom_agent_prompt)

    # 3. Add last user message (with project images attached)
                ...
                for img_file in msg.image_files:
                    if img_file.file_type == ChatFileType.IMAGE:
                        try:
                            ...
                            image_part: ImageContentPart = {
                                "type": "image_url",
                                "image_url": {"url": f"attachment://{img_file.file_id}"}
                            }
